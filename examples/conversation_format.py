...
async def run_simple_conversation():
    ...
    try:
        ...
        await miner.close()
        logger.success("Simple conversation completed successfully!")


async def run_customer_service_and_check_cross_conversation_search():
    config = load_config(group_id=group_id, auto_build_indices=True)
    ...
    try:
        ...
        # Example 3: Multi-turn customer service conversation
        logger.info("Example 3: Multi-turn customer service")
        logger.info("-" * 50)
        ...
        customer_service = [
            ...
            CtxMinerEpisode(
                messages=[
                    CtxMinerMessage(
                        role="user", content="What's included with the Premium 200Mbps?"
                    ),
                    CtxMinerMessage(
                        role="assistant",
        ...
        # Example 4: Searching across all conversations
        logger.info("Example 4: Cross-conversation search")
        logger.info("-" * 50)
...
async def main():
    ...
    # Load configuration
    await run_simple_conversation()
    await run_customer_service_and_check_cross_conversation_search()
